...
//     eventChoiceBox+=`<div class="event-choice" value=${CvID}">${cv_cid_Data.title}</div>`;
//   })
// }
...
inputBox.onkeyup = () => {
  ...
  if (userEnteredValue.trim() != 0) { //if the user value isn't only spaces
    ...
    todoCourseBtn.classList.add("active");
  } else {
    addBtn.classList.remove("active"); //unactive the add button
    todoCourseBtn.classList.remove("active");
  }
}
...
function showTasks() {
  ...
  listArray.forEach((element, index) => {
    newLiTag += `<li value="${element[1]}">${element[0]}<span class="icon" onclick="deleteTask(${index})"><i class="fas fa-trash"></i></span></li>`;
  });
